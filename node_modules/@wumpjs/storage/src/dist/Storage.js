...
export default class Storage extends Map {
  ...
  constructor(options) {
    ...
    if (options?.size && typeof options?.size !== "number") (new StorageError(`'${size}' is not Number.`, { name: "TypeError" })).throw();
     ...
     * Storage options.
     */
    Object.defineProperty(this, "options", { value: options, enumerable: false, writable: false, configurable: false });
    Object.defineProperty(this, "_deleteSet", { value: (key, newKey, value) => { this.delete(key); this.set(newKey, value); }, enumerable: false, configurable: false, writable: false });
  };
   ...
   */
  get clone() {
    const __constructor = new (this.constructor[Symbol.species])({ size: this.options.size });
  ...
  set(key, value) {
    if (this.options.size !== 0 && (this.options.size < this.size)) (new StorageError(`Storage limit exceeded.`, { name: "RangeError" })).throw();
  ...
  getMultiple(...keys) {
    ...
    for (const key of keys) {
      if (typeof key !== "string") (new StorageError(`'${key}' is not String.`, { name: "TypeError" })).throw();
  ...
  findAny(callback, thisArg) {
    ...
    let __results = [];
    for (const [key, value] of this) if (callback(value, key, this)) __results.push({ key, value });
  ...
  filter(callback, thisArg) {
     ...
     * @type {Storage}
     */
    const results = new (this.constructor[Symbol.species])({ size: this.options.size });
  ...
  randomValue(count) {
    if (typeof count !== "number") (new StorageError(`'${count}' is not a number.`, { name: "TypeError" })).throw();
    ...
    for (let i = 0; i < length; i++) {
      const randomIndex = Storage.random({ max: entries.length });
  ...
  valueAt(index) {
    if (typeof index !== "number") (new StorageError(`'${index}' is not a number.`, { name: "TypeError" })).throw();
  ...
  sort(callback = Storage.sort) {
    if (typeof callback !== "function") (new StorageError(`'${callback}' is not a function.`, { name: "TypeError" })).throw();
  ...
  subtract(storage) {
    if (!(storage instanceof this)) (new StorageError(`'${storage}' is not Storage instance.`, { name: "TypeError" })).throw();
  ...
  static random(options) {
    ...
    if ((options?.min && typeof options?.min !== "number") || (options?.max && typeof options?.max !== "number")) (new StorageError(`'${min}' or '${max}' is not Number. `, { name: "TypeError" })).throw();

    if (min >= max) (new StorageError(`'${max}' must be greater than '${min}'. `, { name: "RangeError" })).throw();
